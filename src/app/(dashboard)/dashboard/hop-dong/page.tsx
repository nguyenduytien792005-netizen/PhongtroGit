  ...
  return (
    <div className="space-y-4 md:space-y-6">
      <PageHeader
        ...
        subtitle="Danh sách tất cả hợp đồng trong hệ thống"
        actions={
          <>
          ...
          <Button size="sm" onClick={() => router.push('/dashboard/hop-dong/them-moi')} className={`flex-1 sm:flex-none ${primaryButtonClass}`}>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 md:gap-4">
        <StatCard title="Tổng hợp đồng" value={hopDongList.length} icon={FileText} variant="default" />
        <StatCard title="Hoạt động" value={hopDongList.filter(h => h.trangThai === 'hoatDong').length} icon={FileText} variant="success" />
        <StatCard title="Sắp hết hạn" value={hopDongList.filter(h => isExpiringSoon(h.ngayKetThuc)).length} icon={Calendar} variant="warning" />
        <StatCard title="Đã hết hạn" value={hopDongList.filter(h => isExpired(h.ngayKetThuc)).length} icon={Calendar} variant="danger" />
      </div>
