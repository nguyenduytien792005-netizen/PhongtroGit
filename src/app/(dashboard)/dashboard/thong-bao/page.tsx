  ...
  return (
    <div className="space-y-4 md:space-y-6">
      <PageHeader
        ...
        subtitle="Gửi và quản lý thông báo đến khách thuê"
        actions={
          <>
          ...
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            <DialogTrigger asChild>
              <Button size="sm" onClick={() => setEditingThongBao(null)} className={`flex-1 sm:flex-none ${primaryButtonClass}`}>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 md:gap-4">
        <StatCard title="Tổng thông báo" value={thongBaoList.length} icon={Bell} variant="default" />
        <StatCard title="Chung" value={thongBaoList.filter(t => t.loai === 'chung').length} icon={Bell} variant="info" />
        <StatCard title="Hóa đơn" value={thongBaoList.filter(t => t.loai === 'hoaDon').length} icon={Bell} variant="success" />
        <StatCard title="Sự cố" value={thongBaoList.filter(t => t.loai === 'suCo').length} icon={Bell} variant="danger" />
      </div>
