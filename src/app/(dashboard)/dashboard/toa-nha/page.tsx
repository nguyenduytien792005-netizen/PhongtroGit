  ...
  return (
    <div className="space-y-4 md:space-y-6">
      <PageHeader
        ...
        subtitle="Danh sách tất cả tòa nhà trong hệ thống"
        actions={
          <>
          <Button 
            variant="outline"
          ...
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            <DialogTrigger asChild>
              <Button size="sm" onClick={() => setEditingToaNha(null)} className={`flex-1 sm:flex-none ${primaryButtonClass}`}>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 md:gap-4">
        <StatCard
          title="Tổng tòa nhà"
          value={toaNhaList.length}
          icon={Building2}
          variant="default"
        />
        <StatCard
          title="Phòng trống"
          value={toaNhaList.reduce((sum, toaNha) => sum + ((toaNha as any).phongTrong || 0), 0)}
          icon={Users}
          variant="success"
        />
        <StatCard
          title="Đang thuê"
          value={toaNhaList.reduce((sum, toaNha) => sum + ((toaNha as any).phongDangThue || 0), 0)}
          icon={Users}
          variant="info"
        />
        <StatCard
          title="Tìm thấy"
          value={filteredToaNha.length}
          icon={Search}
          variant="warning"
        />
      </div>
      ...
      <div className="md:hidden space-y-3">
        <div className="flex justify-between items-center">
          ...
          <span className="text-xs text-muted-foreground">{filteredToaNha.length} tòa nhà</span>
        </div>
        ...
        {filteredToaNha.length === 0 ? (
          <Card className="p-6 text-center border-dashed">
            ...
            <h3 className="text-base font-medium text-foreground mb-1">Không tìm thấy tòa nhà nào</h3>
            <p className="text-sm text-muted-foreground">Thử thay đổi tìm kiếm</p>
          </Card>
        ) : (
          <div className="grid grid-cols-1 gap-3">
            {filteredToaNha.map((toaNha) => {
              ...
              return (
                <Card key={toaNha._id} className="hover:shadow-lg transition-all duration-200 border-border/60 hover:border-indigo-200">
                  <CardContent className="p-3">
                    ...
                    <div className="grid grid-cols-3 gap-2 mb-3 p-2 bg-muted/50 rounded-lg">
                      ...
                      <div className="text-center border-x border-border">
                        ...
                        <div className="text-sm font-semibold text-emerald-600">{phongTrong}</div>
                      </div>
                      <div className="text-center">
                        <div className="text-xs text-muted-foreground">Thuê</div>
                        <div className="text-sm font-semibold text-sky-600">{phongDangThue}</div>
                      </div>
                    </div>

                    {toaNha.tienNghiChung && toaNha.tienNghiChung.length > 0 && (
                      <div className="mb-3">
                        <div className="text-xs text-muted-foreground mb-1">Tiện nghi:</div>
