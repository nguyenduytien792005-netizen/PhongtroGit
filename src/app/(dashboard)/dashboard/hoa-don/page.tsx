    ...
    <div className="space-y-4 md:space-y-6">
      <PageHeader
        ...
        actions={
          ...
          <Button size="sm" onClick={() => router.push('/dashboard/hoa-don/them-moi')} className={`flex-1 sm:flex-none ${primaryButtonClass}`}>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 md:gap-4">
        <StatCard title="Tổng hóa đơn" value={hoaDonList.length} icon={Receipt} variant="default" />
        <StatCard title="Chưa thanh toán" value={hoaDonList.filter(h => h.trangThai === 'chuaThanhToan').length} icon={Receipt} variant="danger" />
        <StatCard title="Quá hạn" value={hoaDonList.filter(h => new Date(h.hanThanhToan) < new Date()).length} icon={AlertCircle} variant="warning" />
        <StatCard title="Doanh thu" value={formatCurrency(hoaDonList.reduce((sum, h) => sum + h.daThanhToan, 0))} icon={Receipt} variant="success" />
      </div>
