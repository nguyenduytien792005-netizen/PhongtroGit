  ...
  return (
    <div className="space-y-4 md:space-y-6">
      <PageHeader
        ...
        subtitle="Danh sách tất cả giao dịch thanh toán"
        actions={
          <>
          ...
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            <DialogTrigger asChild>
              <Button size="sm" onClick={() => setEditingThanhToan(null)} className={`flex-1 sm:flex-none ${primaryButtonClass}`}>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 md:gap-4">
        <StatCard title="Tổng giao dịch" value={thanhToanList.length} icon={CreditCard} variant="default" />
        <StatCard title="Tiền mặt" value={thanhToanList.filter(t => t.phuongThuc === 'tienMat').length} icon={CreditCard} variant="success" />
        <StatCard title="Chuyển khoản" value={thanhToanList.filter(t => t.phuongThuc === 'chuyenKhoan').length} icon={CreditCard} variant="info" />
        <StatCard title="Tổng tiền" value={formatCurrency(thanhToanList.reduce((sum, t) => sum + t.soTien, 0))} icon={Receipt} variant="purple" />
      </div>
