...
export default function SuCoPage() {
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      <PageHeader
        title="Quản lý sự cố"
        subtitle="Theo dõi và xử lý các sự cố từ khách thuê"
        actions={
          <>
          ...
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            <DialogTrigger asChild>
              <Button size="sm" onClick={() => setEditingSuCo(null)} className={`flex-1 sm:flex-none ${primaryButtonClass}`}>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 md:gap-4">
        <StatCard title="Tổng sự cố" value={suCoList.length} icon={AlertTriangle} variant="default" />
        <StatCard title="Mới" value={suCoList.filter(s => s.trangThai === 'moi').length} icon={AlertTriangle} variant="danger" />
        <StatCard title="Đang xử lý" value={suCoList.filter(s => s.trangThai === 'dangXuLy').length} icon={Clock} variant="warning" />
        <StatCard title="Đã xong" value={suCoList.filter(s => s.trangThai === 'daXong').length} icon={CheckCircle} variant="success" />
      </div>
